...
#docker的镜像下载地址：https://hub.docker.com/u/kubeguide
...
| 主机名称 | IP地址         | 部署软件                       | 备注   |
| -------- | -------------- | ------------------------------ | ------ |
| k8s-11   | 192.168.10.11  | keepalived+haproxy+etcd+master | master |
| k8s-12   | 192.168.10.12  | keepalived+haproxy+etcd+master | master |
| k8s-13   | 192.168.10.13  | keepalived+haproxy+etcd+master | master |
| k8s-14   | 192.168.10.14  | node+docker                    | work   |
| k8s-15   | 192.168.10.15  | node+docker                    | work   |
| VIP      | 192.168.10.100 |                                | VIP    |
...
ExecStart=/usr/local/bin/etcd \
  ...
  --initial-cluster-token etcd-cluster-0 \
  --initial-cluster kube-11=https://192.168.10.11:2380,kube-12=https://192.168.10.12:2380,kube-13=https://192.168.10.13:2380 \
  --initial-cluster-state new \
  --data-dir=/var/lib/etcd
Restart=on-failure
RestartSec=5
...
#2.查看集群成员
etcdctl --endpoints=https://192.168.10.11:2379,https://192.168.10.12:2379,https://192.168.10.13:2379  --ca-file=/etc/kubernetes/ssl/ca.pem  --cert-file=/etc/kubernetes/ssl/etcd.pem  --key-file=/etc/kubernetes/ssl/etcd-key.pem member list
#返回如下结果
...
#把相关指令和文件拷贝到所有主机
scp /opt/cni/* 192.168.10.12:/usr/local/bin && scp /etc/cni/net.d/* 192.168.10.13:/etc/cni/net.d/
```
...
backend kubernetes-master
    balance roundrobin
    server k8s-m12 192.168.10.11:6443 check maxconn 2000
    server k8s-m13 192.168.10.12:6443 check maxconn 2000
    server k8s-m14 192.168.10.13:6443 check maxconn 2000
EOF
...
scp kube-apiserver kubeadm kube-controller-manager kubectl kube-scheduler 192.168.10.12:/usr/local/bin
...
export BOOTSTRAP_TOKEN=$(kubeadm token create \
  ...
  --groups system:bootstrappers:kube-12 \
...
kubectl config set-cluster kubernetes \
  ...
  --embed-certs=true \
  --server=https://192.168.10.100:8443 \
  --kubeconfig=kubelet-bootstrap-kube-12.kubeconfig
...
#2.5.设置默认上下文
kubectl config use-context default --kubeconfig=kubelet-bootstrap-kube-12.kubeconfig
...
scp kubelet-bootstrap-kube-12.kubeconfig 192.168.10.14:/etc/kubernetes/ssl/kubelet-bootstrap.kubeconfig
scp kubelet-bootstrap-kube-12.kubeconfig 192.168.10.15:/etc/kubernetes/ssl/kubelet-bootstrap.kubeconfig
...
scp kubelet.config.json 192.168.10.14:/etc/kubernetes/ssl
...
#  --allow-privileged=true \	1.15.0以上版本加上这个参数会报错，删除即可
...
#6.遇到报错问题
Unable to register node "192.168.10.14" with API server: nodes "192.168.10.14" is forbidden: node "127.0.0.1" is not allowed to modify node "192.168.10.14"
#解决办法
kubelet.service的IP地址写的是127.0.0.1,请求API的地址报错，后面修改了IP造成了这样，需要先删除证书kube-controller-manager.pem，然后在重启kubelet
```
...
curl -k -s --cacert /etc/kubernetes/ssl/ca.pem https://127.0.0.1:10250/metrics
curl -k -s --cacert /etc/kubernetes/ssl/ca.pem -H "Authorization: Bearer 123456" https://192.168.10.15:10250/metrics
#返回Unauthorized
...
#权限不足的证书；
curl -k -s --cacert /etc/kubernetes/ssl/ca.pem --cert /etc/kubernetes/ssl/kube-controller-manager.pem --key /etc/kubernetes/ssl/kube-controller-manager-key.pem https://192.168.10.15:10250/metrics
#返回如下
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

#使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书；
curl -k -s --cacert /etc/kubernetes/ssl/ca.pem --cert /etc/kubernetes/ssl/admin.pem --key /etc/kubernetes/ssl/admin-key.pem https://192.168.10.15:10250/metrics|head
#返回如下-----------------------
...
curl -k -s --cacert /etc/kubernetes/ssl/ca.pem -H "Authorization: Bearer ${TOKEN}" https://192.168.10.15:10250/metrics|head
...
```bash
#使用浏览器访问https://192.168.10.15:10250/metrics 访问不通，需要使用证书，生成证书方式参考如下：

#1.Windows机器，需要安装jdk然后使用keytool工具
.\keytool -import -v -trustcacerts -alias appmanagement -file "E:\ca.pem" -storepass password -keystore cacerts

#2.然后在linux上执行
openssl pkcs12 -export -out admin.pfx -inkey admin-key.pem -in admin.pem -certfile ca.pem
#3.然后通过浏览器吧证书导进去，就可以正常访问了，导入方式如图
```

![1.png](https://i.loli.net/2019/08/12/gQJWOdV4HywRFBU.png)
![2.png](https://i.loli.net/2019/08/12/xNBt7brFwlp4Ic8.png)
...
scp kube-proxy*.pem kube-proxy.kubeconfig 192.168.10.14:/etc/kubernetes/ssl/
...
#4.检查执行结果
kubectl -n kube-system get pod -o wide|grep -E 'elasticsearch|fluentd|kibana'
...
自动化部署包：	链接：https://pan.baidu.com/s/1ZHGa6iPFIGc_uaijDcoMIw 		提取码：fyul 

```bash
#1.统一安装在/app目录，然后做软连接
ln -s /app/container/kubernetes_ansible /root/kubernetes_ansible

#2.进入目录，修改kubernetes_ha_install_inventory 自动部署文件
cd kubernetes_ansible/kubernetes-inventory/

#3.修改检查脚本kubernetes_check_inventory

#4.进入检查脚本目录，执行安装前检查
cd kubernetes_ansible/kubernetes-check/scripts

#4.1先设置下集群名称
export cluserName=paas-cluster-c1
echo $cluserName	#检查设置的变量

#4.2然后执行检查
sh check_ansible.sh &&
sh check_centos_version.sh &&
sh check_firewalld.sh &&
sh check_handle.sh &&
sh check_kernel.sh &&
sh check_network.sh &&
sh check_python.sh &&
sh check_selinux.sh &&
sh check_ssh_pass.sh &&
sh check_swap.sh

#5.运行自动部署脚本
cd kubernetes_ansible/kubernetes-install

#5.1安装harbor
sh 1-install_ha_harbor.sh	#第一次安装会有红色报错，因为此脚本会先去停止docker，如果未安装docker则会报错，无视即可
#脚本执行结束后需要去harbor镜像库穿件镜像空间（项目），两个harbor里面都需要创建。需要的镜像空间有：kubernetes, paas, monitoring。

#5.2推镜像到harbor
sh 2-push_images_to_harbor.sh	#由于推镜像是通过harbor的vip来推的,所以需要2个harbor镜像库的镜像同步,需要通过配置复制规则来进行同步
#注意：机器需要有docker，并且登陆harbor镜像库

#5.3安装k8s
sh 3-install_ha_kubernetes.sh

#6.安装不报错，然后在执行检查脚本
sh k8s_check_cs.sh &&
sh k8s_check_keepalived.sh &&
sh k8s_check_nginx.sh &&
sh k8s_check_nodes.sh &&
sh k8s_check_pods.sh

#7.没有问题后可以发布应用测试
```

### 3.1、Kubespray部署Kubernetes集群

#### 3.1.1、准备工作

机器规划：[kubespray下载地址](https://github.com/kubernetes-sigs/kubespray/releases)  下载最新的release分支

| IP地址        | 用途                           |
| ------------- | ------------------------------ |
| 192.168.10.11 | ansible  master node kubespray |
| 192.168.10.12 | master node                    |
| 192.168.10.13 | master node                    |
| 192.168.10.14 | node                           |
| 192.168.10.15 | node                           |

[部署参考1](https://dzone.com/articles/kubespray-10-simple-steps-for-installing-a-product)	[部署参考2](https://yq.aliyun.com/articles/505382)	[部署参考3](https://www.cnblogs.com/iiiiher/p/8128184.html)	[部署参考4](https://kubernetes.io/docs/setup/production-environment/tools/kubespray/)	[参考部署5](https://www.kubernetes.org.cn/5012.html)

```shell
#1.安装 ansible
yum install epel-release
yum install ansible		#contos8.0 无法直接yum安装key参考如下方式
#或
...
pip3 install MarkupSafe jinja2 --upgrade
...
#4.配置免密登陆
ssh-keygen -t rsa
ssh-copy-id root@<ip>		#分发到个主机
```

#### 3.1.2、安装kubespray

```shell
#1.我这里使用的是 kubespray-v2.11.0，安装kubespray依赖包
pip3 install -r /kubespray-2.11.0/requirements.txt 
cd /kubespray-2.11.0/inventory/
cp -rp sample/ my-kube-cluster

#2.使用inventory_builder初始化inventory文件，也可以参考原有inventory.ini文件格式
declare -a IPS=(192.168.10.11 192.168.10.12 192.168.10.13 192.168.10.14 192.168.10.15)
CONFIG_FILE=hosts.ini python3 ../../contrib/inventory_builder/inventory.py ${IPS[@]}
#~~~~生产的hosts.ini如下~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
all:
  hosts:
    node1:
...
node1 ansible_host=192.168.10.14  ip=192.168.10.14 #etcd_member_name=etcd4
node2 ansible_host=192.168.10.15  ip=192.168.10.15 #etcd_member_name=etcd5
...
#3.使用ansible playbook部署kubespray
ansible -i inventory/my-kube-cluster/hosts.ini all -m ping		#测试主机连通性
ansible-playbook -i inventory/my-kube-cluster/hosts.ini cluster.yml		#等待一段时间部署完成
```
...
#Core
	...
	cri-o v1.14.0 (experimental: see CRI-O Note. Only on centos based OS)
#Network Plugin
	cni-plugins v0.8.1
	calico v3.7.3
...
#2.镜像
./kubespray-2.11.0/roles/download/defaults/main.yml	#镜像文件
...
[参考：docker Hup 关联 github自动构建镜像](https://jingyan.baidu.com/article/454316ab29dec0f7a7c03a11.html)
