...
kubectl describe nodes		# 查看详细结点信息

kubectl cluster-info	#查看集群信息，运行在那些ip上

kubectl get secret -n kube-system	# 查看master的各种token
kubectl describe secret/[token name] -n kube-system # 查看某一个特定的token

kubectl get svc --all-namespaces	# 查看集群服务状态

kubectl get pod -o wide --all-namespaces		#查看pods所在的运行节点
kubectl get pod -o wide -n 集群空间名 

kubectl get pod -o yaml		#查看pod定义的详细信息
...
kubectl create deployment nginx --image=nginx	#创建一个nginx服务
